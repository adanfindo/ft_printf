...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/02/28 09:31:50 by afindo            #+#    #+#             */
/*   Updated: 2022/03/01 12:56:39 by afindo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_insert	fill_insert(t_elem *elem, t_stack *stack_a, t_stack *stack_b)
{
	...
	if (stack_b->count == 0)
		...
		insert.rb = insert.rb % stack_b->count;
	if (stack_b->count == 0)
		insert.rrb = 0;
	else
		insert.rrb = ((stack_b->count - insert.rb) % stack_b->count);
	nb_op(&insert);
	return (insert);
}
